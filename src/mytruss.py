...
def get_ktrussProbs(graph):
    #print ("Extracting KTruss features: %s" % name)
    start_time = time.time()
    trussness = ktruss(graph)
    ktrusstiming = time.time() - start_time
    
    edgetrussness = trussness.values()
    m = graph.ecount()
    n = graph.vcount()
    maxedgetruss = max(edgetrussness)
    d = {n:edgetrussness.count(n) for n in range(2,max(edgetrussness)+1)} 
    edgetrussprobability = [d[key] / (m * 1.0) for key in sorted(d)]
    ...
    for edge in graph.es:
        ...
        if not (source == target) :
           t = trussness[(source,target)]
        else:
           t = 0		  
        nodetrussness[source] = max(nodetrussness[source], t)
        nodetrussness[target] = max(nodetrussness[target], t)
    
    d = {n:nodetrussness.count(n) for n in range(2,max(nodetrussness)+1)} 
    nodetrussprobability = [d[key] / (m * 1.0) for key in sorted(d)]

    EdgeMatrix = [[0 for x in range(maxedgetruss+1)] for x in range(maxedgetruss+1)] 
    for edge in graph.es:
       i = nodetrussness[edge.source]
       j = nodetrussness[edge.target]
       #print("Edge:", e.source,coreness[e.source],"-", e.target,coreness[e.target])
       EdgeMatrix[i][j] += 1
       if (i != j):
         EdgeMatrix[j][i] += 1
    
    intrainterEdgeprobabilities = []   
    if m == 0:
      m = 1
    for i in range(maxedgetruss+1):
       for j in range(i+1):
          intrainterEdgeprobabilities.append(EdgeMatrix[i][j]/float(m))

    return nodetrussprobability,edgetrussprobability, intrainterEdgeprobabilities, ktrusstiming
...
def TrussDistribution(graphs):
    ...
    for g in graphs:
       ...
       print 'm = ',len(G.edges())
       start_time = time.time()
       KTrussSignature = get_ktrussProbs(G)
...
def getedgediversity(graph):
    ktrussdict = ktruss(graph)
    nodediversity = [0] * graph.vcount()
    entropy = [0] * graph.vcount()
    for edge in graph.es:
      source = edge.source		
      target = edge.target
      if not (source == target) :
    ...
    #nodediversity = graph.diversity(weights = graph.es['weight'])
    #nodediversity = [0 if x != x else x for x in nodediversity]   #replace nan with 0
    
    for v in graph.vs() :
      #print 'name: ',v['name']
      #print 'num incident edges: ', len(graph.incident(v.index))
      edgetrusses = [graph.es[edge]['weight'] for edge in graph.incident(v.index)]
      #print 'trussness of edges: ', edgetrusses
      ent = 0
      numedges = len(edgetrusses)
      numclasses = len(set(edgetrusses))
      if edgetrusses :
             maxedgetruss = max(edgetrusses) 
             probs = [(edgetrusses.count(n)*1.0)/numedges for n in range(max(edgetrusses)+1)]   #numneighbors
             probs = [p for p in probs if p > 0.]
             parray = np.array(probs)
             ent = - parray.dot(np.log2(parray))
             if not(maxedgetruss-1) == 1:
                ent = ent/np.log2(maxedgetruss-1)
             entropy[v.index] = ent 
      #print 'entropy of edges: ', entropy[v.index]
      #print 'nodediversity: ', nodediversity[v.index]
    return entropy
...
def gettrussscore(g):    
    ...
    outfilename = directory + graphname + ".trussscore"
